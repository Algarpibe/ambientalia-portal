      ...
      {q.isLoading ? <p className="text-gray-500">Cargando…</p> : q.error ? <p className="text-red-600">{(q.error as Error).message}</p> : rows.length === 0 ? <p className="text-gray-500">Sin datos.</p> : (
        <div className="overflow-x-auto">
          <table className="text-xs border-collapse">
            <thead><tr><th className="sticky left-0 bg-[#FCFBF9] px-2 py-1 text-left">Cliente</th>{MONTHS.map((m) => <th key={m} className="px-2 py-1 text-center">{m}</th>)}</tr></thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.customer}>
                  <td className="sticky left-0 bg-[#FCFBF9] px-2 py-1 whitespace-nowrap max-w-[180px] truncate">{r.customer}</td>
                  {r.months.map((v, i) => (
